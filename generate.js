const ASSETS_PATH = "fluentui-emoji/assets";
const ASSETS_OUT_PATH = "dist/assets";
const METADATA_OUT_PATH = "dist";
...
(async () => {
  ...
  await Promise.map(
    ...
    async (id) => {
      const metadata = JSON.parse(
        fs.readFileSync(path.join(ASSETS_PATH, id, "metadata.json"))
      );
      let asset = "";
      for (const dir of [
        ...
        if (fs.existsSync(dir)) {
          ...
          const scaled = await sharp(assetPath)
            .resize(128)
            .webp({ lossless: true })
            .toBuffer();
          const hash = crypto
  ...
  fs.writeFileSync(
    path.join(METADATA_OUT_PATH, "metadata.json"),
